...
public class WeatherUtil {
	 ...
	 * @param T_p lookahead time, relative to timeOfCurrentPosition.  Conflicts beyond this time will be ignored.
	...
	static public Pair<Plan,DensityGrid> reRouteWx(Plan own, ArrayList<PolyPath> paths, double gridSize, double buffer, 
		...
		if (paths.size() == 0) {
			...
			//DebugSupport.dumpAsUnitTest(nPlan);	
			//			for (int i = 0; solution == null && i < 3; i++) {
			//				String speedString = "";
			//				if (i == 1) {
			//					gs = nPlan.initialVelocity(0).gs() - maxGroundSpeedDelta*nPlan.initialVelocity(0).gs();
			//					speedString = " with SLOWER speed";
			//				} else if (i == 2) {
			//					gs = nPlan.initialVelocity(0).gs() + maxGroundSpeedDelta*nPlan.initialVelocity(0).gs();
			//					speedString = " with FASTER speed";
			//				}
			//				if (maxGroundSpeedDelta == 0.0) {
			//					i += 3; // break out if no change allowed
			//				}
			// f.pln(" $$ reRoute: nPlan = "+nPlan);
			//PolyPath pp0 = paths.get(0);
			//dumpPolyPathAsUnitTest(paths);				
			//StratUtil.dumpPolyPathAsUnitTest(containment,"Containment");
			double endT = -1.0;
...
//f.pln(" $$$$ reRouteWx: rr.second = "+rr.second);
			if (rrPlan.size() > 0) {
				...
				//DebugSupport.dumpAsUnitTest(rrPlan);
				//DebugSupport.dumpPlan(rrPlan, "rrPlan");
				if (rrPlan.size() > 0) {
					...
					//DebugSupport.dumpPlan(solution, "solution");
					solution = rrPlan;
				} else {
...
//f.pln(" reRouteWx: rrPlan = "+rrPlan);			
	 ...
	 * @param T_p lookahead time for search, relative to timeOfCurrentPosition.  conflicts after this are ignored
	...
	static public Pair<Plan,DensityGrid> reRouteWx(Plan own, ArrayList<PolyPath> paths, double gridSize, double buffer, 
		...
		//DebugSupport.dumpPlan(own,"reRouteWx_plan");
		//DebugSupport.dumpPolyPaths(paths,"reRouteWx_polys");
		//DebugSupport.dumpPolyPaths(npaths,"reRouteWx_polys_stretched");
		...
		if (reRouteReduction && ret != null) {
			...
			if (ret2 != null) {
				ret = ret2;
            //DebugSupport.dumpPlan(ret, "tbodynrnpbuggyOUT");				
			}
	...
	static private Pair<Plan,DensityGrid> reRouteWithAstar(ArrayList<PolyPath> paths, Plan ownship, double gridSize, double buffer, 
		...
		//DebugSupport.dumpDensityGrid(dg, "gridcorners", ownship.getFirstTime());
		if (gPath == null) {
