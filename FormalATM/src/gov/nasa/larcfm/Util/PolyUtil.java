...
//			}
	 ...
	 * @param increment time for search (> 0)
	 * @return true if the point mass will intersect with the polygon between times B and T
	 */
	public static boolean intersectsPolygon2D(Plan p, PolyPath pp, double B, double T, double incr) {
		if (incr <= 0) return false;
		...
		for (double t = start; t <= end; t += incr) {
			if (pp.position(t).contains2D(p.position(t))) {
...
//f.pln("lastsz="+lastsz+" currsz="+curr.size());			
			...
			while (i < curr.size()-1) {
				...
				if (isPlanClear2D(tmp, paths, start, end, incr)) {
	...
	/**
	 * Returns true if the plan is free of polygons from time start to time end.  This may miss intrusions of up to incr sec
	 ...
	 * @return
	 */
	public static boolean isPlanClear2D(Plan plan, ArrayList<PolyPath> paths, double start, double end, double incr) {
		for (int i = 0; i < paths.size(); i++) {
			if (intersectsPolygon2D(plan, paths.get(i), start, end, incr)) {
				return false;
			}
		}
		return true;
	...
	public static boolean isPlanClear2D(Plan plan, ArrayList<PolyPath> paths, double incr) {
		for (int i = 0; i < paths.size(); i++) {
			if (intersectsPolygon2D(plan, paths.get(i), plan.getFirstTime(), plan.getLastTime(), incr)) {
				return false;
			}
		}
		return true;
...
//			// non-collinear intersections
//			ArrayList<Triple<Integer,Integer,Position>> intersectionPoints = new ArrayList<Triple<Integer,Integer,Position>>();
...
//	}
//	
//	public static ArrayList<Poly2D> grid2polys (boolean[][] grid, int xsz, int ysz) {
