 ...
 * this is using the Util.TrajectoryGen.makeKinematicPlan() method to
 * construct a kinematic Plan core from a linear one (and use
 * Util.TrajectoryGen.makeLinearPlan to do the inverse). <p>
 *
...
public class Plan implements ErrorReporter, Cloneable {
	...
	public double getTime(int i) {
		if (i < 0 || i >= size()) {
			//      System.out.println("$$!! invalid point index of "+i+" in getTime()");
...
//			DebugSupport.halt();
	 ...
	 */
	public boolean timeshiftPlan(int start, double dt) {
		return timeshiftPlan(start, dt, false);
	}
	 ...
	 */
	public boolean timeshiftPlan(int start, double dt, boolean preserveRTAs) {
		//f.pln(" $$$$ timeshiftPlan: "+start+" "+size() );
		if (Double.isNaN(dt)) {
			if (debug) DebugSupport.halt();
			return false;
		}
	...
	public Velocity initialVelocity(int i, boolean linear) { // ***RWB*** Kchange
	...
	private Velocity getDtVelocity(int i) {
		...
		//		if (j == (int) points.size()) { // past the end, need to go back a bit
		//		j--;
		...
		if (j >= points.size()) { // special case, back up a bit
			...
			//			Thread.dumpStack();
			return p.initialVelocity(points.get(size()-1).position(), 10.0*minDt);
		}
     ...
     *  If checkSource is true, this function checks to make sure that the source position is reasonably close to the current position.  
     *  and if not, it reverts to the current position of "dSeg".
     *  
...
//f.pln("Plan.closestPointHoriz iterative best = ("+nnp+") dist="+p.distanceH(nnp.position()));
		...
		if (pathDistance(seg) <= 0.0) {
			//f.pln("closestPositionHoriz: patDistance for seg "+seg+" is zero");
			//f.pln(toString());
			return np;
		}
		NavPoint ret;
		// linear cases
		if (inTrkChange(t1)) {
//f.pln("Plan.closestPointHoriz inTrkChange");		
...
//f.pln("Plan.closestPointHoriz ret="+ret);		
	...
	public NavPoint closestPoint(Position p) {
		return closestPoint(0,points.size()-1, p);
	}
	 ...
	 * @return
	 */
	public NavPoint closestPoint(int start, int end, Position p) {
	...
	public List<String> toStringList(int i, int precision, boolean tcp) {
		List<String> ret = point(i).toStringList(precision, tcp);
		return ret;
	}
