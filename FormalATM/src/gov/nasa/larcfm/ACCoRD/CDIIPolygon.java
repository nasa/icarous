...
//        f.pln(" $$## CDIIPolygon.detectionLL: cdsi = "+cdsi);
   ...
   * @return
   */
  public boolean conflictDetection(Plan ownship, PolyPath traffic, double B, double T) {    
	    if (ownship.isLatLon() != traffic.isLatLon()) {
	      return false;
	    }
	    if (ownship.isLatLon()) {
   ...
   * @return
   */
  private boolean conflictXYZ(Plan ownship, PolyPath traffic, double B, double T) {
	    int start = B > ownship.getLastTime() ? ownship.size()-1 : Math.max(0,ownship.getSegment(B));
	    int end = T > ownship.getLastTime() ? ownship.size()-1 : Math.max(0,ownship.size()-1);
	    for (int i = start; i < ownship.size(); i++){
	      if (i <= end) {
   ...
   * @return
   */
	  private boolean conflictLL(Plan ownship, PolyPath traffic, double B, double T) {
	    int start = B > ownship.getLastTime() ? ownship.size()-1 : Math.max(0,ownship.getSegment(B));
	    int end = T > ownship.getLastTime() ? ownship.size()-1 : Math.max(0,ownship.size()-1);
	    for (int i = start; i < ownship.size(); i++){ 
	      if (i <= end) {
