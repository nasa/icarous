...
TEST_F(TrajGenTest, testRemoveShortLegs2) {
	...
	Plan kpc = TrajGen::makeKinematicPlan(lpc, bankAngle, gsAccel, vsAccel, true, true, true);
	bool silent = false;
	EXPECT_TRUE(kpc.isConsistent(silent));
...
TEST_F(TrajGenTest, testShortLegs2) {
	...
	lpc.addNavPoint(np3);
	//DebugSupport.dumpPlan(lpc, "testShortLegs2_lpc");
	Plan kpc = TrajGen::makeKinematicPlan(lpc, 1.3*bankAngle, gsAccel, vsAccel, repair, repair, repair);
	EXPECT_TRUE(kpc.isFlyable());
	EXPECT_TRUE(PlanUtil::basicCheck(lpc,kpc,20));
...
//		fpln(" $$$$ test_AWE918. msg ="+kpc.getMessageNoClear());
	...
	//fpln("\n ----------------------------------------");
	kpc = TrajGen::generateVsTCPs(lpc,vsAccel,continueGen,false);
...
//		EXPECT_EQ("VERT",kpc.point(1).name());make
...
TEST_F(TrajGenTest, test_T007) {
	...
	bool continueGen = false;
	Plan vpc = TrajGen::markVsChanges(lpc);
...
TEST_F(TrajGenTest, test_Dp) {
	...
	Plan kpc = TrajGen::generateVsTCPs(lpc,vsAccel,continueGen,false);
}
...
TEST_F(TrajGenTest, test_UF_3783) {  // point within turn ends up extremely close to added EOT
    ...
    EXPECT_EQ("CC",kpc.getName(3));
    EXPECT_EQ("DD",kpc.getName(4));
    EXPECT_EQ("Vert4",kpc.getName(5));   // THE MOT
    EXPECT_EQ("FF",kpc.getName(6));
    EXPECT_EQ("GG",kpc.getName(8));
