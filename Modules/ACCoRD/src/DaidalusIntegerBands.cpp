...
int DaidalusIntegerBands::first_los_step(const Detection3D* det, double ts,bool trajdir,
    int min, int max, const TrafficState& ownship, const TrafficState& traffic) const {
  for (int k=min; k<=max; ++k) {
    if (LOS_at(det,trajdir,k*ts,ownship,traffic)) {
      return k;
    }
  }
  return -1;
...
int DaidalusIntegerBands::bands_search_index(const Detection3D* conflict_det, const Detection3D* recovery_det, double tstep,
    ...
    bool trajdir, int max, const TrafficState& ownship, const TrafficState& traffic,
...
bool DaidalusIntegerBands::no_CD_future_traj(const Detection3D* conflict_det, const Detection3D* recovery_det, double B, double T, double B2,
    double T2, bool trajdir, double tsk, const TrafficState& ownship, const TrafficState& traffic) const {
  return !CD_future_traj(conflict_det,B,T,trajdir,tsk,ownship,traffic) &&
      !(recovery_det != NULL && CD_future_traj(recovery_det,B2,T2,trajdir,tsk,ownship,traffic));
}
...
void DaidalusIntegerBands::traj_conflict_only_bands(std::vector<Integerval>& l,
    ...
    bool trajdir, int max, const TrafficState& ownship, const TrafficState& traffic) const {
  int d = -1; // Set to the first index with no conflict
  for (int k = 0; k <= max; ++k) {
    double tsk = tstep*k;
    if (d >=0 && no_CD_future_traj(conflict_det,recovery_det,B,T,B2,T2,trajdir,tsk,ownship,traffic)) {
      continue;
    } else if (d >=0) {
      l.push_back( Integerval(d,k-1));
      d = -1;
    } else if (no_CD_future_traj(conflict_det,recovery_det,B,T,B2,T2,trajdir,tsk,ownship,traffic)) {
      d = k;
    }
  }
  if (d >= 0 && d != max) {
...
void DaidalusIntegerBands::kinematic_bands(std::vector<Integerval>& l, const Detection3D* conflict_det, const Detection3D* recovery_det, double tstep,
    ...
    int epsh, int epsv) const {
  l.clear();
  int bsi = bands_search_index(conflict_det,recovery_det,tstep,B,T,B2,T2,trajdir,max,ownship,traffic,epsh,epsv);
  if  (bsi != 0) {
...
void DaidalusIntegerBands::kinematic_bands_combine(std::vector<Integerval>& l, const Detection3D* conflict_det, const Detection3D* recovery_det, double tstep,
    double B, double T, double B2, double T2,
    int maxl, int maxr, const TrafficState& ownship, const TrafficState& traffic,
    int epsh, int epsv) const {
  kinematic_bands(l,conflict_det,recovery_det,tstep,B,T,B2,T2,false,maxl,ownship,traffic,epsh,epsv);
  std::vector<Integerval> r;
  kinematic_bands(r,conflict_det,recovery_det,tstep,B,T,B2,T2,true,maxr,ownship,traffic,epsh,epsv);
  neg(l);
  append_intband(l,r);
}
...
int DaidalusIntegerBands::first_green(const Detection3D* conflict_det, const Detection3D* recovery_det, double tstep,
  ...
  for (int k=0; k <= max; ++k) {
    double tsk = tstep*k;
    if ((tsk >= B && tsk <= T && LOS_at(conflict_det,trajdir,tsk,ownship,traffic)) ||
        (recovery_det != NULL && tsk >= B2 && tsk <= T2 &&
            LOS_at(recovery_det,trajdir,tsk,ownship,traffic)) ||
...
bool DaidalusIntegerBands::all_int_red(const Detection3D* conflict_det, const Detection3D* recovery_det, double tstep,
  ...
  bool leftans = dir > 0 || first_green(conflict_det,recovery_det,tstep,B,T,B2,T2,false,maxl,ownship,traffic,epsh,epsv) < 0;
  bool rightans = dir < 0 || first_green(conflict_det,recovery_det,tstep,B,T,B2,T2,true,maxr,ownship,traffic,epsh,epsv) < 0;
...
int DaidalusIntegerBands::first_instantaneous_green(const Detection3D* conflict_det, const Detection3D* recovery_det,
  ...
  for (int k = 0; k <= max; ++k) {
    j_step_ = k;
    if (no_instantaneous_conflict(conflict_det,recovery_det,B,T,B2,T2,trajdir,ownship,traffic,epsh,epsv)) {
      return k;
    }
  }
  return -1;
...
bool DaidalusIntegerBands::all_instantaneous_red(const Detection3D* conflict_det, const Detection3D* recovery_det,
  ...
  bool leftans = dir > 0 || first_instantaneous_green(conflict_det,recovery_det,B,T,B2,T2,false,maxl,ownship,traffic,epsh,epsv) < 0;
  bool rightans = dir < 0 || first_instantaneous_green(conflict_det,recovery_det,B,T,B2,T2,true,maxr,ownship,traffic,epsh,epsv) < 0;
  return leftans && rightans;
}


Vect3 DaidalusIntegerBands::linvel(const TrafficState& ownship, double tstep, bool trajdir, int k) const {
...
bool DaidalusIntegerBands::repulsive_at(double tstep, bool trajdir, int k, const TrafficState& ownship, const TrafficState& traffic, int epsh) const {
  // k >= 0
  if (k==0) {
    return true;
  }
...
int DaidalusIntegerBands::first_nonrepulsive_step(double tstep, bool trajdir, int max, const TrafficState& ownship, const TrafficState& traffic, int epsh) const {
  for (int k=0; k <= max; ++k) {
...
bool DaidalusIntegerBands::vert_repul_at(double tstep, bool trajdir, int k, const TrafficState& ownship, const TrafficState& traffic, int epsv) const {
...
int DaidalusIntegerBands::first_nonvert_repul_step(double tstep, bool trajdir, int max, const TrafficState& ownship, const TrafficState& traffic, int epsv) const {
  for (int k=0; k <= max; ++k) {
...
bool DaidalusIntegerBands::any_conflict_step(const Detection3D* det, double tstep, double B, double T, bool trajdir, int max,
    const TrafficState& ownship, const TrafficState& traffic) const {
  for (int k=0; k <= max; ++k) {
    if (CD_future_traj(det,B,T,trajdir,tstep*k,ownship,traffic)) {
      return true;
    }
  }
  return false;
...
bool DaidalusIntegerBands::any_int_red(const Detection3D* conflict_det, const Detection3D* recovery_det, double tstep,
    ...
    int maxl, int maxr, const TrafficState& ownship, const TrafficState& traffic,
    int epsh, int epsv, int dir) const {
  bool leftred = dir <= 0 && red_band_exist(conflict_det,recovery_det,tstep,B,T,B2,T2,false,maxl,ownship,traffic,epsh,epsv);
  bool rightred = dir >= 0 && red_band_exist(conflict_det,recovery_det,tstep,B,T,B2,T2,true,maxr,ownship,traffic,epsh,epsv);
...
bool DaidalusIntegerBands::any_instantaneous_red(const Detection3D* conflict_det, const Detection3D* recovery_det,
    ...
    int maxl, int maxr, const TrafficState& ownship, const TrafficState& traffic,
    int epsh, int epsv, int dir) {
  bool leftred = dir <= 0 && instantaneous_red_band_exist(conflict_det,recovery_det,B,T,B2,T2,false,maxl,ownship,traffic,epsh,epsv);
  bool rightred = dir >= 0 && instantaneous_red_band_exist(conflict_det,recovery_det,B,T,B2,T2,true,maxr,ownship,traffic,epsh,epsv);
  return leftred || rightred;
}
...
void DaidalusIntegerBands::instantaneous_bands_combine(std::vector<Integerval>& l, const Detection3D* conflict_det, const Detection3D* recovery_det,
    ...
    int maxl, int maxr, const TrafficState& ownship, const TrafficState& traffic,
    int epsh, int epsv) {
  instantaneous_bands(l,conflict_det,recovery_det,B,T,B2,T2,false,maxl,ownship,traffic,epsh,epsv);
  std::vector<Integerval> r;
  instantaneous_bands(r,conflict_det,recovery_det,B,T,B2,T2,true,maxr,ownship,traffic,epsh,epsv);
  neg(l);
  append_intband(l,r);
}
...
bool DaidalusIntegerBands::no_instantaneous_conflict(const Detection3D* conflict_det, const Detection3D* recovery_det, double B, double T, double B2, double T2,
    bool trajdir, const TrafficState& ownship, const TrafficState& traffic,
    int epsh, int epsv) const {
  bool usehcrit = epsh != 0;
  bool usevcrit = epsv != 0;
  ...
  return
      ...
      no_CD_future_traj(conflict_det,recovery_det,B,T,B2,T2,trajdir,0.0,ownship,traffic);
}
...
void DaidalusIntegerBands::instantaneous_bands(std::vector<Integerval>& l,
  ...
  for (int k = 0; k <= max; ++k) {
    j_step_ = k;
    if (d >=0 && no_instantaneous_conflict(conflict_det,recovery_det,B,T,B2,T2,trajdir,ownship,traffic,epsh,epsv)) {
      continue;
    } else if (d >=0) {
      Integerval iv = Integerval(d,k-1);
      l.push_back(iv);
      d = -1;
    } else if (no_instantaneous_conflict(conflict_det,recovery_det,B,T,B2,T2,trajdir,ownship,traffic,epsh,epsv)) {
      d = k;
    }
  }
  if (d >= 0 && d != max) {
