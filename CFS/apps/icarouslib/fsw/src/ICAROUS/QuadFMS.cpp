 ...
 */
 ...
 FlightManagementSystem_t(&fData),Detector(this),Resolver(this){
     targetAlt         = 0.0f;
...
uint8_t QuadFMS_t::TAKEOFF(){
	...
	if(sendTkCommand < 0){
		targetAlt = (float) FlightData->paramData->takeoffAlt;
		...
		sendTkCommand = FlightData->acTime;
	}else if(sendTkCommand > 0){
		if( (FlightData->acTime - sendTkCommand) > 2){
...
uint8_t QuadFMS_t::Resolve(){
	...
	case COMPUTE_r:
		...
		if(Detector.trafficConflict){
			...
			int search_type = FlightData->paramData->cheapDAA;
			if(search_type == 0){
			...
			int search_type = FlightData->paramData->cheapSearch;
...
uint8_t QuadFMS_t::FlyTrajectory(){
	...
	case START_t:
		...
		if(FlightData->ResolutionPlan.size() > 1){
			FlightData->nextResolutionWP = 1;
		}else{
			FlightData->nextResolutionWP = 0;
		}
		resolutionSpeed = FlightData->paramData->resolutionSpeed;
	...
	case ENROUTE_t:
		...
		captureH  = FlightData->paramData->Capture_H;
		captureV  = FlightData->paramData->Capture_V;
...
uint8_t QuadFMS_t::FlyManuever(){
	uint8_t status = 0;
	float resolutionSpeed = FlightData->paramData->resolutionSpeed;
	uint8_t allowYaw = FlightData->paramData->allowYaw;
	countVelCmd++;
...
void QuadFMS_t::ComputeInterceptCourse(){
	...
	float speed  = FlightData->paramData->resolutionSpeed;
...
uint8_t QuadFMS_t::TRACKING(Position target){
	...
	double heading = FlightData->paramData->trkHeading;
	double distH   = FlightData->paramData->trkDistH;
	double distV   = FlightData->paramData->trkDistV;
	int yawRel      =FlightData->paramData->allowYaw;
