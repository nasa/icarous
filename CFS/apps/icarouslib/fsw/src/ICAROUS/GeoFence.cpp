...
Geofence_t::Geofence_t(int ID, FENCE_TYPE ftype, uint16_t nVert, double infloor, double inCeiling,icarous_table_t* pData){
	fenceType = ftype;
	id        = ID;
	nVertices = nVert;
	floor     = infloor;
...
void Geofence_t::AddVertex(int index, double lat,double lon){
	...
	double ResolBUFF = params->hThreshold; // expansion/contraction amount
...
void Geofence_t::CheckViolation(AircraftState acState,double elapsedTime,Plan fp){
	...
	double hthreshold = params->hThreshold;
	double vthreshold = params->vThreshold;
	double hstepback  = params->hStepback;
	double vstepback  = params->vStepback;
	...
	if(fenceType == KEEP_IN){
		if(geoPolyCarp.nearEdge(currentPosR3,geoPoly3D,hthreshold,vthreshold)){
			...
			conflict = false;
			//conflict = CollisionDetection(currentPosLLA,currentVel.vect2(),0,lookahead);
		}
...
bool Geofence_t::CheckWPFeasibility(Position currentPos,Position nextWP){
	...
	bool val = CollisionDetection(currentPos,vel,0,100);
