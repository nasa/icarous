...
void ConflictDetection_t::CheckFlightPlanDeviation(bool devAllowed){
	...
	double allowedDev = FlightData->paramData->xtrkDev;
	Plan CurrentFP;
...
void ConflictDetection_t::CheckTraffic(){
	...
	double radius = FlightData->paramData->cylRadius;
	double height = FlightData->paramData->cylHeight;
	double alertTime = FlightData->paramData->alertTime;
	double earlyAlertTime = FlightData->paramData->earlyAlertTime;
	double holdConflictTime = FlightData->paramData->conflictHold;
	...
	//DAA.parameters.setLookaheadTime(FlightData->paramData->daaLookAhead);
	...
	if(FlightData->visBands.numBands > 0){
		...
		else if(FlightData->visBands.numBands == 1 && FlightData->visBands.type1 == 0){
			// Don't send if we only have one type 0 band
			// This is to save bandwidth
			FlightData->visBands.numBands = 0;
		}else{
