...
public class GeoFence {
    ...
    public void CheckViolation(AircraftState acState, double currentTime, Plan FP) {
        ...
        // edge:"+geoPoly3D.distanceFromEdge(so));
        // Keep in geofence
        if (Type == 0) {
    ...
    public boolean CheckWaypointFeasibility(Position CurrPos, Position NextWP) {
        ...
        for (int i = 0; i < numVertices; i++) {
            ...
            // **
            InPlaneInt = LinePlaneIntersection(geoPoly3D.getVertex(vertexi), geoPoly3D.getVertex(vertexj), floor,
                    ceiling, CurrPosVec, NextWPVec);
