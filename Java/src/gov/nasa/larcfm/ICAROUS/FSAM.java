...
public class FSAM {
    ...
    public FSAM(Aircraft ac, Mission ms) {
        ...
        // Create an object of type Daidalus for a well-clear volume based on
        // TAUMOD
        daa = new Daidalus();
    ...
    static public AlertLevels AlertQuad(double radius, double height, double alerttime1, double alerttime2) {
        ...
        alertor.addLevel(new AlertThresholds(new CDCylinder(radius, "m", height, "m"), alerttime1, alerttime2,
                BandsRegion.NEAR));
        return alertor;
    }

    // Function to monitor conflicts (Geofence, flight plan deviation, air
    // traffic etc...)
    public FSAM_OUTPUT Monitor() {
        ...
        // Check for deviation from prescribed flight profile only in the NOOP
        // state.
        CheckStandoff();
        ...
        // If conflicts are detected, initialize the state machine for the
        // resolution function
    ...
    public int Resolve() {
        ...
        switch (resolveState) {
            ...
            case COMPUTE:
                ...
                // [TODO:] Add conflict resolution table to add
		    ...
		    UAS.SetMode(4);
                    ...
                    UAS.comIntf.SendStatusText("Keep in conflict");
                    ...
                    UAS.comIntf.SendStatusText("Keep out conflict");
                    ...
                    UAS.comIntf.SendStatusText("Stand off conflict");
                    UsePlan = ResolveStandoffConflict();
                } else {
                    UAS.error.addWarning("[" + UAS.timeLog + "] MSG: No resolution");
                }

                // Two kinds of actions to resolve conflicts : 1. Plan based
                // resolution, 2. Maneuver based resolution
                if (UsePlan) {
    ...
    public void ExecuteManeuver() {
        ...
        switch (executeState) {
            ...
            case PAUSE:
                ...
                // System.out.println("PAUSE:"+(float)((UAS.timeCurrent -
                // pausetime_start))/1E9);
    ...
    // Returns time corresponding to the current position in the resolution
    // flight plan
    ...
    public void CheckGeoFences() {
        ...
        if (UsePlan) {
                ...
                if (GF.conflict) {
                    if (GF.Type == 0) {
                        cf = new Conflict(PRIORITY_LEVEL.MEDIUM, CONFLICT_TYPE.KEEP_IN, GF,
                                FlightData.acState.positionLast());
            ...
            // While executing maneuvers, project current position based on
            // track and ground speed and
    ...
    public void CheckTraffic() {
        ...
        for (int ac = 1; ac < daa.numberOfAircraft(); ac++) {
            ...
            if (tlos >= 0 && tlos <= daaLookahead) {
                ...
                // System.out.printf("Predicted violation with traffic aircraft
                // %s in %.1f [s]\n",
            ...
            if (TrafficConflict) {
                ...
                // "+TS.size()+","+daa.getOwnshipState().formattedTraffic(TS,0));
            }
    ...
    public void ResolveKeepOutConflict() {
        ...
        // Reroute flight plan
        UAS.SetMode(4); // Set mode to guided for quadrotor to hover before
        // replanning
        ...
        ArrayList<PolyPath> LPP = new ArrayList<PolyPath>(); // List of keep out
        // fences
        ArrayList<PolyPath> CPP = new ArrayList<PolyPath>(); // List of keep in
        // fences
        ...
        // Get flight plan between start time and end time (with a 3 second
        ...
        // Get start and end positions based on conflicted parts of the original
        // flight plan
    ...
    public boolean ResolveStandoffConflict() {
        ...
        if (StandoffConflict) {
            ...
            if (Math.abs(crossTrackDeviation) <= 2 * standoff) {
                ...
                // System.out.println("Ref heading:"+RefHeading2);
                UsePlan = false;
                return false; // false indicated that we don't have to use a
                // resolution plan
            } else {
                ...
                // System.out.println("Ref heading in standoff resolution
                // plan:"+RefHeading2);
                ResolutionPlan.clear();
                ...
                UsePlan = true;
                return true; // true indicates that we should use a resolution
                // plan
            }
        } else {
            Vn2 = 0;
    ...
    public void ResolveTrafficConflict() {
        ...
        // Determine which resolution to use (left/right) based on proximity to
	...
	/*
        ...
        if (collisionLeft && !collisionRight) {
            ...
            // Pick the resolution angle that is closest to the next waypoint
            // heading
            if (d1 <= d2) {
        ...
        // If the next waypoing heading is within the <NONE> band, ignore
        // resolution headings
        ...
        // If resolution heading angles available, compute Vn,Ve based on
        // resolution heading
        if (!Double.isNaN(res_heading)) {
            ...
            // If resolution heading unavailable, follow last know resolution
            // heading
    ...
    public void ResolveTrafficFenceConflict() {
        ...
        /*
		 * double alt = currentPos.alt(); double alt_upper =
		 * KMB.altitudeResolution(true,1); double alt_lower =
		 * KMB.altitudeResolution(false,1);
		 * System.out.println("Lower:"+alt_lower+" Upper:"+alt_upper); Vn1 = 0;
		 * Ve1 = 0; Vu1 = 0; if(!Double.isNaN(alt_upper)){ if(alt <= alt_upper){
		 * Vn1 = 0; Ve1 = 0; Vu1 = -3; } } else if(!Double.isNaN(alt_lower)){
		 * if(alt > alt_lower){ Vn1 = 0; Ve1 = 0; Vu1 = 1; } }
         */
    }
