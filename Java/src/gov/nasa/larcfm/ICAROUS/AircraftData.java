...
public class AircraftData {
	...
	public int startMission; // -1: last command executed, 0 - stop mission, 1 -
								// start mission
	...
	public msg_mission_ack SendFlightPlanToAP(Interface Intf) {
		...
		for (int i = 0; i < InputFlightPlan.size(); i++) {
			...
			if ((msgMissionItem0.command == MAV_CMD.MAV_CMD_NAV_WAYPOINT)
				...
				double wptime = 0;
				Position nextWP = Position.makeLatLonAlt(msgMissionItem0.x, "degree", msgMissionItem0.y, "degree",
						msgMissionItem0.z, "m");
				if (i > 0) {
		...
		while (!writeComplete) {
			...
			case FP_SEND_WP:
				...
				if (msgMissionRequest != null) {
					...
					// msgMissionItem.x =
					// (float)CurrentFlightPlan.point(msgMissionItem.seq).lla().latitude();
					// msgMissionItem.y =
					// (float)CurrentFlightPlan.point(msgMissionItem.seq).lla().longitude();
					// msgMissionItem.z =
					// (float)CurrentFlightPlan.point(msgMissionItem.seq).lla().alt();

					Intf.Write(msgMissionItem);
					// System.out.println("Wrote mission item:"+count);
					// System.out.format("lat, lon, alt:
				...
				if (msgMissionAck2 != null) {
					...
					// System.out.println("Received acknowledgement - type:
					// "+Inbox.MissionAck().type);
					SetPauseDAQ(false);
	...
	public int GetWaypoints(Interface Intf, int target_system, int target_component, int COUNT,
			List<msg_mission_item> mission) {
		...
		while (!readComplete) {
			...
			case FP_WP_READ:
				...
				if (msgMissionItem != null) {
					...
					// System.out.println("mission sequence
					// received:"+msgMissionItem.seq);
					if (msgMissionItem.seq == count) {
						...
						// msgMissionAck.type =
						// MAV_MISSION_RESULT.MAV_MISSION_INVALID_SEQUENCE;
		...
		while (!readComplete) {
			...
			case GF_READ:
				...
				if (msgFencePoint != null) {
					...
					// System.out.println("geofence point
					// received:"+msgFencePoint.idx);
					if (msgFencePoint.idx == count) {
						...
						// msg_command_ack msgCommandAck = new
						// msg_command_ack();
