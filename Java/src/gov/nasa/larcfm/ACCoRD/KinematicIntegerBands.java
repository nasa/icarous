...
public abstract class KinematicIntegerBands {
	...
	public boolean all_instantaneous_red(Detection3D conflict_det, Optional<Detection3D> recovery_det, 
			...
			int maxl, int maxr, TrafficState ownship, List<TrafficState> traffic, TrafficState repac,
			int epsh, int epsv, int dir) {
		boolean leftans = dir > 0 || first_instantaneous_green(conflict_det,recovery_det,B,T,B2,T2,false,maxl,ownship,traffic,repac,epsh,epsv) < 0;
		boolean rightans = dir < 0 || first_instantaneous_green(conflict_det,recovery_det,B,T,B2,T2,true,maxr,ownship,traffic,repac,epsh,epsv) < 0;
		return leftans && rightans; 
	}
	...
	public boolean any_instantaneous_red(Detection3D conflict_det, Optional<Detection3D> recovery_det, 
			...
			int maxl, int maxr, TrafficState ownship, List<TrafficState> traffic, TrafficState repac,
			int epsh, int epsv, int dir) {
		boolean leftred = dir <= 0 && instantaneous_red_band_exist(conflict_det,recovery_det,B,T,B2,T2,false,maxl,ownship,traffic,repac,epsh,epsv);
		boolean rightred = dir >= 0 && instantaneous_red_band_exist(conflict_det,recovery_det,B,T,B2,T2,true,maxr,ownship,traffic,repac,epsh,epsv);
		return leftred || rightred; 
	}
	...
	private boolean no_instantaneous_conflict(Detection3D conflict_det, Optional<Detection3D> recovery_det, double B, double T, double B2, double T2,
			boolean trajdir, TrafficState ownship, List<TrafficState> traffic, TrafficState repac,
			int epsh, int epsv) {
		boolean usehcrit = repac.isValid() && epsh != 0;
		boolean usevcrit = repac.isValid() && epsv != 0;    
