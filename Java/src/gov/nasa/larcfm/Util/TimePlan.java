...
//		vadd(time,alt,Double.NaN,vz,PointTypeV.VSFIX, Position.ZERO_XYZ.mkAlt(alt), time, label);
	...
	public double getFirstTime() {
		...
		return Math.min(vmap.firstKey(), hmap.firstKey());
	}
	...
	public double getLastTime() {
		...
		return Math.max(vmap.lastKey(), hmap.lastKey());
	}
	...
	private void addMidPoints(Plan p, int num, double timeBuffer) {
		...
		while (i < p.size()) {
			...
			if (np.isEndTCP()) {
				double t = np.time();
				int j = i-1;
				if (np.isEOT()) j = Math.min(p.prevBOT(i),j);//fixed
				if (np.isEGS()) j = Math.min(p.prevBGS(i),j);//fixed
				if (np.isEVS()) j = Math.min(p.prevBVS(i),j);//fixed
				double prevT = p.getTime(j);
				double dt = t-prevT;
				double step = dt/(1+Math.min(num, Math.floor(dt/timeBuffer)));
				if (step > 0) {
