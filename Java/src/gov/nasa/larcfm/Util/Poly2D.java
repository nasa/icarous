...
//			return false;
//		}
//f.pln(" ^^$$>> Poly2D.contains:  poly2 = "+this+" a="+a+" b="+b);
...
//f.pln("Poly2D.contains: outside BB");
			return false;
		}
...
//	}
	...
	public double signedArea() {
		...
		for (int i = 1 ; i < size - 1 ; ++i) {
			temp += (vertices.get(i).x-dx) * ((vertices.get(i + 1).y-dy) - (vertices.get(i - 1).y-dy));
		}
		double area2 = (vertices.get(0).x-dx) * ((vertices.get(1).y-dy) - (vertices.get(size - 1).y-dy)) 
		                 	+ temp + (vertices.get(size - 1).x-dx) * ((vertices.get(0).y-dy) - (vertices.get(size - 2).y-dy));
		return area2 / 2;
	}
	...
	public double area() {
		...
		for (int i = 0; i < vertices.size()-1; i++) {
			a = a + ((vertices.get(i).x()-dx)*(vertices.get(i+1).y()-dy) - (vertices.get(i+1).x()-dx)*(vertices.get(i).y()-dy));
		}
...
//		f.pln(" $$$$$ outerRadius: cpos = "+cpos);
	...
	public Vect2 centroid()	{
		...
		for (int i = 0 ; i < size - 1 ; ++i) {
			double x0 = vertices.get(i).x - dx;
			double y0 = vertices.get(i).y - dy;
			double x1 = vertices.get(i+1).x - dx;
			double y1 = vertices.get(i+1).y - dy;
			
			temp = x0 * y1 - x1 * y0;
			tempX += (x0 + x1) * temp;
			tempY += (y0 + y1) * temp;
		}
		double x0 = vertices.get(0).x - dx;
		double y0 = vertices.get(0).y - dy;
		double x1 = vertices.get(size - 1).x - dx;
		double y1 = vertices.get(size - 1).y - dy;
		...
		tempX /= area6;
		tempY /= area6;
		return new Vect2(tempX+dx, tempY+dy);
	}
	...
	public Vect2 averagePoint() {
		...
		double y = 0;
		for (int i = 0; i < vertices.size(); i++) {
			x += vertices.get(i).x()-dx;
			y += vertices.get(i).y()-dy;
		}
		return new Vect2(dx+x/vertices.size(), dy+y/vertices.size());
	}
...
//	}
	...
	public String toString() {
		return "Poly2D [vertices=" + vertices+ 
