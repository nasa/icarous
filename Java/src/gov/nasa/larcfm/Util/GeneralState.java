...
public class GeneralState {
	...
	public GeneralState(String name, SimplePoly p, Velocity v, double time, boolean cont) {
		id = name;
		si = null;
		vi = null;
		mp = new SimpleMovingPoly(p,v);
		t = time;
		containment = cont;		
	}
	...
	public General3DState get3DState(EuclideanProjection proj) {
		if (si != null) {
			...
			state = new General3DState(mp.getMovingPolygon(proj), containment);
			return state;
		} else {
