...
public class PolycarpResolution {
	...
	public static ArrayList<Vect2> expand_polygon_2D(double BUFF, ArrayList<Vect2> p) {
		...
		for (int i = 0; i < p.size(); i++) {
			...
			Vect2 q = p.get(i).Add(R.Scal(2*BUFF));
	...
	public static ArrayList<Vect2> contract_polygon_2D(double BUFF, ArrayList<Vect2> p) {
		ArrayList<Vect2> p2 = new ArrayList<Vect2>();
		for (int i = 0; i < p.size(); i++) {
			Vect2 prev = i > 0 ? p.get(i-1) : p.get(p.size()-1);
			Vect2 next = i < p.size()-1 ? p.get(i+1) : p.get(0);
			Vect2 pv = proj_vect(prev, p.get(i), next, BUFF);
			Vect2 R = p.get(i).Sub(prev).PerpR().Hat();
			Vect2 q = p.get(i).Sub(R.Scal(2*BUFF));
			double tt = (Math.abs(pv.dot(R)) <= BUFF/100) ? 0 : q.Sub(p.get(i)).dot(R)/pv.dot(R);
			p2.add(p.get(i).Add(pv.Scal(tt)));
		}
		return p2;
	...
	public static Vect2 recovery_test_point(double BUFF, ArrayList<Vect2>p, Vect2 s, int eps) {
		...
		Vect2 testvect = ip.Add(testdir.Scal(eps*2*BUFF));
		return testvect;
	}

	public static Vect2 recovery_point(double BUFF, ArrayList<Vect2> p, Vect2 s, int eps) {
		Vect2 tv = recovery_test_point(BUFF,p,s,eps);
		if (eps == 1 && PolycarpContain.definitely_outside(p,tv,BUFF)) return tv;
		if (eps == -1 && PolycarpContain.definitely_inside(p, tv, BUFF)) return tv;
		int i = closest_edge(p,BUFF,s);
		int nexti = i < p.size()-1 ? i+1 : 0;
		...
		Vect2 q = p.get(neari).Add(R.Scal(eps*2*BUFF));
		double tt = Math.abs(pv.dot(R)) <= BUFF/100 ? 0 : q.Sub(p.get(i)).dot(R)/pv.dot(R);
		if ((eps == 1 && leftturn) || eps == -1 && !leftturn) {
			return p.get(neari).Add(pvnormed.Scal((2*BUFF)));
		} else {
			return p.get(neari).Add(pv.Scal(tt));
		}
		
	}
	
	public static Vect2 outside_recovery_point(double BUFF, ArrayList<Vect2> p, Vect2 s) {
		return recovery_point(BUFF,p,s,1);
	}
	
	public static Vect2 inside_recovery_point(double BUFF, ArrayList<Vect2> p, Vect2 s) {
		return recovery_point(BUFF,p,s,-1);
	}
