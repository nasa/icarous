...
//	    return ans
	...
	public static boolean corner_lt_3deg(Vect2 v,Vect2 w,double BUFF) {
	    if (v.norm()<2*BUFF || w.norm()<2*BUFF) return true;
	...
	public static boolean acceptable_polygon_2D(ArrayList<Vect2> p,double BUFF) {
	    ...
	    for (int i = 0; i < p.size(); i++) {
	        for (int j = 0; j < p.size(); j++) {
	            ...
	            if (j==mi && corner_lt_3deg(pj.Sub(pi),pmj.Sub(pj),BUFF)) return false;
	            if (j==mi) continue;
	            if (i==mj && (near_poly_edge(p,pmi,BUFF,j) || near_poly_edge(p,pj,BUFF,i))) return false;
	            if (i==mj && corner_lt_3deg(pi.Sub(pj),pmi.Sub(pi),BUFF)) return false;
	            if (i==mj) continue;
