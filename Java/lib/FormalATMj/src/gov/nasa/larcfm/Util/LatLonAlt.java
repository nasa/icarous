...
//	}
  ...
  /**
    ...
    */
	 ...
	 * Example: 39 degrees, 20.01 minutes north latitude, 85 degrees, 10.53 minutes west longitude is represented as N392001W0851053
	 * @return
	 */
	public String toStringDecimalMinutes() {
		...
		double min_lat = (deg_lat - Math.floor(deg_lat)) * 60;
		sb.append(f.FmLead((int)Math.floor(min_lat),2));

		double dmin_lat = Math.round((min_lat - Math.floor(min_lat)) * 100);
		sb.append(f.FmLead((int)Math.floor(dmin_lat),2));
		...
		double min_lon = (deg_lon - Math.floor(deg_lon)) * 60;
		sb.append(f.FmLead((int)Math.floor(min_lon),2));

		double dmin_lon = Math.round((min_lon - Math.floor(min_lon)) * 100);
		sb.append(f.FmLead((int)Math.floor(dmin_lon),2));
