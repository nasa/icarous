...
void TRACKER_AppInit(void) {
    ...
    status = CFE_SB_CreatePipe(&trackerAppData.tracker_Pipe, /* Variable to hold Pipe ID */
    ...
    status = CFE_TBL_Register(&trackerAppData.tracker_tblHandle,
                              "trackerTable",
    ...
    // Load app table data
    status = CFE_TBL_Load(trackerAppData.tracker_tblHandle,CFE_TBL_SRC_ADDRESS,&tracker_TblStruct);

    trackerTable_t *TblPtr;
    status = CFE_TBL_GetAddress((void**)&TblPtr, trackerAppData.tracker_tblHandle);

    trackerAppData.TargetTracker = new TargetTracker(NULL);
