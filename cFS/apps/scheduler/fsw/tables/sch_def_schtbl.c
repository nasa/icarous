
/*==============================================================================
** File Name:  sch_def_schtbl.c
**
** Title:  CFS scheduler table
**
** $Author:    Auto-generated by gen_sch_tbl
** $Revision: 1.1 $
** $Date: 2014/08/28 12:53:20GMT-05:00 $
**
** Purpose:  
**
**  Schedule table configuration parameters:
**    SCH_MAX_MINOR_FRAMES 100
**    MSGS_PER_MINOR_FRAMES 5
**    APP_ID    PRIO    RATE_Hz    INITIAL_MF    MSG_TYPE(WU_HK)
**    INTERFACE_GS    61    50    0    WU
**    INTERFACE_AP    60    50    0    WU
**    PLEXIL    62    30    1    WU
**    GEOFENCE    63    10    2    WU
**    TRAFFIC    63    10    3    WU
**    TRAJECTORY    64    10    4    WU
**    TRACKING    65    10    4    WU

**============================================================================*/
/*
** Pragmas
*/

/*
** Include Files
*/
#include "cfe.h"
#include "cfe_tbl_filedef.h"
#include "sch_platform_cfg.h"
#include "sch_msgdefs.h"
#include "sch_tbldefs.h"
#include "sch_grpids.h"
#include "sch_tblidx.h"
/*
** Local Defines
*/

/*
** Local Structure Declarations
*/

/*
** External Global Variables
*/

/*
** Global Variables
*/

/* Table file header */
static CFE_TBL_FileDef_t CFE_TBL_FileDef =
{
    "SCH_DefaultScheduleTable", "SCH.SCHED_DEF", "SCH schedule table",
    "sch_def_schtbl.tbl", (sizeof(SCH_ScheduleEntry_t) * SCH_TABLE_ENTRIES)
};

/* Default schedule table data */
SCH_ScheduleEntry_t SCH_DefaultScheduleTable[SCH_TABLE_ENTRIES] =
{
    /*
    ** Structure definition...
    **
    **    uint8    EnableState  -- SCH_UNUSED, SCH_ENABLED, SCH_DISABLED
    **    uint8    Type         -- 0 or SCH_ACTIVITY_SEND_MSG
    **    uint16   Frequency    -- how many seconds between Activity execution
    **    uint16   Remainder    -- seconds offset to perform Activity
    **    uint16   MessageIndex -- Message Index into Message Definition table
    **    uint32   GroupData    -- Group and Multi-Group membership definitions
    */
    
    /* Slot #0 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #1 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #2 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, GEOFENCE_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #3 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAFFIC_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #4 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAJECTORY_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRACKING_WAKEUP_MIDX, SCH_GROUP_MISC },

    /* Slot #5 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #6 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #7 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #8 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #9 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #10 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #11 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #12 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, GEOFENCE_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #13 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAFFIC_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #14 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAJECTORY_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRACKING_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #15 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #16 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #17 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #18 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #19 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #20 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #21 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #22 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, GEOFENCE_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #23 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAFFIC_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #24 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAJECTORY_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRACKING_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #25 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #26 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #27 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #28 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #29 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #30 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #31 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #32 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, GEOFENCE_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #33 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAFFIC_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #34 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAJECTORY_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRACKING_WAKEUP_MIDX, SCH_GROUP_MISC },

    /* Slot #35 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #36 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #37 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #38 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #39 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #40 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #41 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #42 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, GEOFENCE_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #43 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAFFIC_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #44 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAJECTORY_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRACKING_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #45 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #46 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #47 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #48 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #49 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #50 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #51 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #52 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, GEOFENCE_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #53 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAFFIC_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #54 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAJECTORY_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRACKING_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #55 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #56 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #57 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #58 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #59 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #60 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #61 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #62 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, GEOFENCE_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #63 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAFFIC_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #64 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAJECTORY_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRACKING_WAKEUP_MIDX, SCH_GROUP_MISC },

    /* Slot #65 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #66 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #67 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #68 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #69 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #70 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #71 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #72 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, GEOFENCE_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #73 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAFFIC_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #74 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAJECTORY_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRACKING_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #75 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #76 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #77 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #78 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #79 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #80 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #81 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #82 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, GEOFENCE_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #83 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAFFIC_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #84 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAJECTORY_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRACKING_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #85 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #86 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #87 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #88 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #89 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #90 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #91 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #92 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, GEOFENCE_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #93 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAFFIC_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #94 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRAJECTORY_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, TRACKING_WAKEUP_MIDX, SCH_GROUP_MISC },

    /* Slot #95 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #96 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #97 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, PLEXIL_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #98 */
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_GS_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_ENABLED, SCH_ACTIVITY_SEND_MSG, 1, 0, INTERFACE_AP_WAKEUP_MIDX, SCH_GROUP_MISC },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },

    /* Slot #99 */
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
    { SCH_UNUSED, 0, 0, 0, 0, SCH_GROUP_NONE },
};

/*==============================================================================
** End of file sch_def_schtbl.c
**============================================================================*/
    